...
class Hrac:
    def __init__(self, zadane_jmeno, zadana_sirka, zadana_pozice_x, zadana_pozice_y, barva):
    ...
    def zobraz(self, platno_kam):
        kolecko_obdelnik = pygame.draw.circle(platno_kam, self.barva, [self.pozice_x, self.pozice_y], self.prumer // 2,
                                              0)
...
mraky = pygame.image.load("Images/mrak.jpg").convert()
mraky = pygame.transform.scale(mraky, (OKNO_SIRKA, OKNO_VYSKA))

# >>> Funkce convert() upravi data tak, aby je Pygame mohl rychle vykreslovat -> tedy ji chceme používat :-)
auto = pygame.image.load("Images/car.png").convert_alpha()
auto_obdelnik = auto.get_rect()
auto = pygame.transform.scale(auto, (auto_obdelnik[2]//5, auto_obdelnik[3]//5))
...
platno_hlavni.blit(mraky, (0, 0))

hrac1_obdelnik = hrac1.zobraz(platno_hlavni)
#hrac2_obdelnik = hrac2.zobraz(platno_hlavni)

auto_obdelnik = platno_hlavni.blit(auto, dest=(50, 400), area=None)
...
running = True
while running:
    # Ukladá si do fronty všechny události a zpracuje je ve chvíli, kdy doběhne na tento řádek (tedy i ty, které se staly kdykoliv v průběhu programu)
    ...
    textRect = text.get_rect()
    textRect.center = (OKNO_SIRKA // 2, textRect[2])  # textRect[2] výška obdélníku
    platno_hlavni.blit(mraky, dest=textRect_toRemove, area=textRect_toRemove)
    textRect_toRemove = platno_hlavni.blit(text, dest=textRect)
    ...
    # pracuje s autem
    platno_hlavni.blit(mraky, dest=auto_obdelnik, area=auto_obdelnik)  # "ctrl+C ctrl+V"
    auto_obdelnik = platno_hlavni.blit(auto, dest=(50, 400), area=None)  # Vloží celý (area=None) obrázek auto do platno_hlavni na pozici (50, 200)

    # pracuje s koleckem
    platno_hlavni.blit(platno_hlavni, dest=hrac1_obdelnik, area=hrac1_obdelnik)  # "ctrl+C ctrl+V"
    kolecko_obdelnik = hrac1.zobraz(platno_hlavni)
